import React, { useEffect } from "react";
import AOS from 'aos';
import 'aos/dist/aos.css';

const Workflow1 = () => {
  useEffect(() => {
    AOS.init({
      duration: 1000, // Duration of animations
      easing: 'ease-in-out', // Easing function
      once: true, // Animation only happens once
    });
  }, []);

  return (
    <div className="flex flex-col gap-2 lg:px-12 py-10 w-full bg-white">
      <h1 className="text-2xl font-semibold text-center" data-aos="fade-up">How It Works?</h1>

      <div className="flex flex-col gap-4 mx-auto">
        <div className="md:flex xs:flex-col gap-4 items-center">
          <div className="md:flex xs:flex-col items-center gap-4">
            <p className="border-2 border-black rounded-md w-72 sm:w-44 h-32 text-center py-8 bg-yellow-400 text-white" data-aos="fade-up">
              Set Details about Pooja and States
            </p>
            <img
              src="https://th.bing.com/th/id/R.fdeb923d963b131c3df59e878b8ed81c?rik=9I1RcMp7F3i5RA&riu=http%3a%2f%2fwww.clipartbest.com%2fcliparts%2f9cR%2fb6L%2f9cRb6L9bi.jpg&ehk=BVJmloJisu9dMbB%2fQTPVUel7iBembgbtfbSnaK%2bYHqE%3d&risl=&pid=ImgRaw&r=0"
              width={100}
              className="hidden md:block"
              data-aos="fade-left"
            />
            <img
              src="https://th.bing.com/th/id/R.fdeb923d963b131c3df59e878b8ed81c?rik=9I1RcMp7F3i5RA&riu=http%3a%2f%2fwww.clipartbest.com%2fcliparts%2f9cR%2fb6L%2f9cRb6L9bi.jpg&ehk=BVJmloJisu9dMbB%2fQTPVUel7iBembgbtfbSnaK%2bYHqE%3d&risl=&pid=ImgRaw&r=0"
              width={100}
              className="md:hidden sm:block transform rotate-90 my-6 mx-auto"
              data-aos="fade-right"
            />
          </div>

          <div className="md:flex xs:flex-col items-center gap-4 justify-center">
            <p className="border-2 border-black rounded-md w-72 sm:w-44 h-32 text-center py-6 px-2 bg-yellow-400 text-white" data-aos="fade-up">
              Set your Required Time Schedule
            </p>
            <img
              src="https://th.bing.com/th/id/R.fdeb923d963b131c3df59e878b8ed81c?rik=9I1RcMp7F3i5RA&riu=http%3a%2f%2fwww.clipartbest.com%2fcliparts%2f9cR%2fb6L%2f9cRb6L9bi.jpg&ehk=BVJmloJisu9dMbB%2fQTPVUel7iBembgbtfbSnaK%2bYHqE%3d&risl=&pid=ImgRaw&r=0"
              width={100}
              className="hidden md:block"
              data-aos="fade-left"
            />
            <img
              src="https://th.bing.com/th/id/R.fdeb923d963b131c3df59e878b8ed81c?rik=9I1RcMp7F3i5RA&riu=http%3a%2f%2fwww.clipartbest.com%2fcliparts%2f9cR%2fb6L%2f9cRb6L9bi.jpg&ehk=BVJmloJisu9dMbB%2fQTPVUel7iBembgbtfbSnaK%2bYHqE%3d&risl=&pid=ImgRaw&r=0"
              width={100}
              className="md:hidden sm:block transform rotate-90 my-6 mx-auto"
              data-aos="fade-right"
            />
          </div>

          <div className="flex flex-col gap-4 items-center md:pt-24 xs:pt-0">
            <p className="border-2 border-black rounded-md w-72 sm:w-44 h-32 text-center pt-12 bg-yellow-400 text-white" data-aos="fade-up">
              Search For the Pooja Booking
            </p>
            <img
              src="https://th.bing.com/th/id/R.fdeb923d963b131c3df59e878b8ed81c?rik=9I1RcMp7F3i5RA&riu=http%3a%2f%2fwww.clipartbest.com%2fcliparts%2f9cR%2fb6L%2f9cRb6L9bi.jpg&ehk=BVJmloJisu9dMbB%2fQTPVUel7iBembgbtfbSnaK%2bYHqE%3d&risl=&pid=ImgRaw&r=0"
              width={100}
              className="transform rotate-90"
              data-aos="fade-left"
            />
          </div>
        </div>

        <div className="md:flex xs:flex-col gap-4 items-center">
          <div className="md:flex xs:flex-col items-center gap-4">
            <p className="border-2 border-black rounded-md w-72 sm:w-44 h-32 text-center py-8 bg-yellow-400 text-white" data-aos="fade-up">
              Go for the Data if available
            </p>
            <img
              src="https://th.bing.com/th/id/R.fdeb923d963b131c3df59e878b8ed81c?rik=9I1RcMp7F3i5RA&riu=http%3a%2f%2fwww.clipartbest.com%2fcliparts%2f9cR%2fb6L%2f9cRb6L9bi.jpg&ehk=BVJmloJisu9dMbB%2fQTPVUel7iBembgbtfbSnaK%2bYHqE%3d&risl=&pid=ImgRaw&r=0"
              width={100}
              className="hidden md:block transform rotate-180"
              data-aos="fade-left"
            />
            <img
              src="https://th.bing.com/th/id/R.fdeb923d963b131c3df59e878b8ed81c?rik=9I1RcMp7F3i5RA&riu=http%3a%2f%2fwww.clipartbest.com%2fcliparts%2f9cR%2fb6L%2f9cRb6L9bi.jpg&ehk=BVJmloJisu9dMbB%2fQTPVUel7iBembgbtfbSnaK%2bYHqE%3d&risl=&pid=ImgRaw&r=0"
              width={100}
              className="md:hidden sm:block transform rotate-90 my-6 mx-auto"
              data-aos="fade-right"
            />
          </div>

          <div className="md:flex xs:flex-col items-center gap-4">
            <p className="border-2 border-black rounded-md w-72 sm:w-44 h-32 text-center py-12 bg-yellow-400 text-white" data-aos="fade-up">
              Click on Book now after Reading all info
            </p>
            <img
              src="https://th.bing.com/th/id/R.fdeb923d963b131c3df59e878b8ed81c?rik=9I1RcMp7F3i5RA&riu=http%3a%2f%2fwww.clipartbest.com%2fcliparts%2f9cR%2fb6L%2f9cRb6L9bi.jpg&ehk=BVJmloJisu9dMbB%2fQTPVUel7iBembgbtfbSnaK%2bYHqE%3d&risl=&pid=ImgRaw&r=0"
              width={100}
              className="hidden md:block transform rotate-180"
              data-aos="fade-left"
            />
            <img
              src="https://th.bing.com/th/id/R.fdeb923d963b131c3df59e878b8ed81c?rik=9I1RcMp7F3i5RA&riu=http%3a%2f%2fwww.clipartbest.com%2fcliparts%2f9cR%2fb6L%2f9cRb6L9bi.jpg&ehk=BVJmloJisu9dMbB%2fQTPVUel7iBembgbtfbSnaK%2bYHqE%3d&risl=&pid=ImgRaw&r=0"
              width={100}
              className="md:hidden sm:block transform rotate-90 my-6 mx-auto"
              data-aos="fade-right"
            />
          </div>

          <div>
            <p className="border-2 border-black rounded-md w-72 h-32 sm:w-44 text-center py-12 bg-yellow-400 text-white" data-aos="fade-up">
              Fill the form and Book
            </p>
          </div>
        </div>
      </div>
    </div>
  );
};

export default Workflow1;
