...
const navigate = useNavigate()
  ...
  const handleSuggestionClick = (suggestion) => {
    setBirthPlace(suggestion.description);
    setSuggestions([]);
  };
  ...
  return (
    <div className="md:w-3/5 xs:w-full flex flex-col gap-3">
      ...
      <form className="p-3 border-[1px] rounded-lg flex flex-col gap-4 shadow-md">
        ...
        <button className="border mt-2 rounded-full py-2 text-yellow-400 bg-gray-800 hover:bg-yellow-400 hover:text-black transition duration-400 hover:border shadow-xl" onClick={()=>navigate("/freekundali/basic-details")}>
          Generate Horoscope
        </button>
      </form>
    </div>
