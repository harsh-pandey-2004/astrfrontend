...
const VimshottariDashaTab = () => {
  ...
  useEffect(()=>{
    const fetchDasha=async()=>{
      try{
        const res=await fetch(`https://api.vedicastroapi.com/v3-json/dashas/maha-dasha-predictions?dob=${formData.day}/${formData.month}/${formData.year}&tob=${formData.hour}:${formData.minute}&lat=${userLat}&lon=${userLong}&tz=5.5&api_key=9e300788-82de-534a-ba13-47e11a18669f&lang=en`);
