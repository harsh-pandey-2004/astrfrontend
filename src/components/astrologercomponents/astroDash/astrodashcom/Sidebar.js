...
function Sidebar({ response }) {
  ...
  return (
    <div>
      <div className="sidebar w-full    h-screen flex flex-col items-center overflow-y-auto ">
        ...
        <div className="flex flex-col items-center gap-4 pt-4  text-center">
          ...
          <div className="py-2 px-3 rounded-full border hover:bg-yellow-400 transition hover:cursor-pointer hover:text-white text-sm w-full  bg-slate-100 ">
            {" "}
            <Link to={`/panditdashboard/${response._id}/shedule`}>
              Schedule{" "}
            </Link>
          </div>
      ...
      {isShow && <div className="w-full h-screen bg-black"></div>}
    </div>
  );
}
